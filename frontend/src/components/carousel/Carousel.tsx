...
function Carousel({
    ...
    const getDot = (i: number) => {
        ...
        if (renderDot && dots) {
            const offset =
                props.centerMode && !props.infinity
                    ? Math.floor(showSlides / 2)
                    : 0;

            el = renderDot({
                selected: props.centerMode
                    ? (i + offset) % rows.length ===
                      carousel.centerIndex % rows.length
                    : i === carousel.slide,
                index: i + offset,
            });
            el = React.cloneElement(el, {
                ...el.props,
                onClick: () => carousel.toSlide(i - 1),
    ...
    const rootSx = React.useMemo(() => {
        ...
        return (theme: any) =>
            objectsAssign(sxStyles as any, (sx as any)(theme));
    }, [sx]);
